...
void _drawPolyLines(Image& image, Rcpp::List line, bool isClosed,
  ...
  if (image.GPU) {
    cv::polylines(image.uimage, lines, isClosed, col2Scalar(color), thickness);
  } else {
...
void _fillPoly(Image& image, Rcpp::List polygon, Rcpp::NumericVector color) {
  ...
  if (image.GPU) {
    cv::fillPoly(image.uimage, polygons, col2Scalar(color));
  } else {
...
void _fillConvexPoly(Image& image, Rcpp::IntegerMatrix polygon, Rcpp::NumericVector color) {
  if (image.GPU) {
    cv::fillConvexPoly(image.uimage, rmat2poly(polygon), col2Scalar(color));
  } else {
...
void _inpaint(Image& image, Image& mask, double radius, int method, Image& target) {
  if (image.GPU && mask.GPU && target.GPU) {
    cv::inpaint(image.uimage, mask.uimage, target.uimage, radius, method);
  } else if (!image.GPU && !mask.GPU && !target.GPU) {
    cv::inpaint(image.image, mask.image, target.image, radius, method);
  } else {
    Rcpp::stop("'image$GPU', 'mask$GPU', and 'target$GPU' are not equal.");
  }
}
...
void _setTo(Image& image, Image& mask, NumericVector color) {
  if (image.GPU && mask.GPU) {
    ...
    Rcpp::stop("'image$GPU' and 'mask$GPU' are not equal.");
  }
}